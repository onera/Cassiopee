...
def cart2Cyl(a, center=(0,0,0), axis=(0,0,1), depth=0):
    """Transform a mesh defined in Cartesian coordinates into cylindrical coordinates.
    Usage: cart2Cyl(a, center, axis)"""
    b = Converter.copy(a)
    _cart2Cyl(b, center, axis, depth=depth)
    return b

def _cart2Cyl(a, center=(0,0,0), axis=(0,0,1), depth=0):
    if isinstance(a[0], list):
        b = []
        for i in a:
            b.append(transform._cart2CylA(i, center, axis, depth))
        return b
    else:
        return transform._cart2CylA(a, center, axis, depth)
