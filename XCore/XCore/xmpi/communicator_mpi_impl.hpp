...
#       endif
    ...
    struct communicator::Implementation {
                             ...
                             int *tr_ranks ) const
        {
            ...
            int error = MPI_Group_translate_ranks( group1, nbRanks, ranks, group2, tr_ranks );
            if ( error != MPI_SUCCESS )
                std::cerr << "Warning : error translating ranks.(" << error << ")" << std::endl;
        ...
        struct Communication {
            ...
            static status recv( const MPI_Comm &com, K &rcvobj, int sender, int tag )
            {
                status status;
                if ( Type_MPI<K>::must_be_packed() ) {
                    MPI_Recv( &rcvobj, sizeof( K ), MPI_BYTE, sender, tag, com, &status.mpi_status );
                } else {
                    MPI_Recv( &rcvobj, 1, Type_MPI<K>::mpi_type(), sender, tag, com, &status.mpi_status );
                }
                return status;
            }
            // .......................................................................................
            ...
            // .......................................................................................
            static void gather( const MPI_Comm &com, const K& loc, K *gather_array, int root )
            {
                MPI_Gather( &loc, 1, Type_MPI<K>::mpi_type(), gather_array, 1, Type_MPI<K>::mpi_type(), root, com );
            }
        };
        // ---------------------------------------------------------------------------------------------
        ...
        status recv( std::size_t nbItems, K *rcvbuff, int sender, int tag ) const
        {
            status status;
            if ( Type_MPI<K>::must_be_packed() ) {
                MPI_Recv( rcvbuff, nbItems * sizeof( K ), MPI_BYTE, sender, tag, m_communicator, &status.mpi_status );
            } else {
                MPI_Recv( rcvbuff, nbItems, Type_MPI<K>::mpi_type(), sender, tag, m_communicator, &status.mpi_status );
            }
            return status;
        }
        // .........................................................................................
        ...
        void reduce( std::size_t nbItems, const K *objs, K *res, Operation op, int root )
        {
            ...
            else MPI_Reduce( objs, res, nbItems, Type_MPI<K>::mpi_type(), op, root, m_communicator );
        }
        // .........................................................................................
        template <typename K>
...
#           if __cplusplus >= 201402L            
            ...
            MPI_Op_free( &op );
#           else
            assert(false && "Only available with a c++ compiler compliant with C++ 14 or greater");
#           endif
        }
        // ====================================================================================================
        template <typename K>
...
#           endif
        ...
        {
            assert( objs != nullptr );
            MPI_Gather( objs, nbItems, Type_MPI<K>::mpi_type(), res, nbItems, Type_MPI<K>::mpi_type(), root, m_communicator );
        }
        // .........................................................................................
        template <typename K>
        void gather( const K &loc, K *glob, int root ) const
        {
            Communication<K, is_container<K>::value>::gather( m_communicator, loc, glob, root );
        }
    private:
    ...
    struct communicator::Implementation::Communication<K, true> {
        ...
        static status recv( const MPI_Comm &com, K &rcvobj, int sender, int tag )
        {
            ...
            if ( Type_MPI<typename K::value_type>::must_be_packed() ) {
                MPI_Recv( rcv->data(), rcv->size() * sizeof( typename K::value_type ), MPI_BYTE, sender, tag, com,
                          &st );
            } else {
                MPI_Recv( rcv->data(), rcv->size(), Type_MPI<typename K::value_type>::mpi_type(), sender, tag, com,
                          &st );
            }
            if ( !std::is_base_of<std::vector<typename K::value_type, typename K::allocator_type>, K>::value ) {
        ...
        static void reduce( const MPI_Comm &com, const K &loc, K *glob, const Operation &op, int root )
        {
            ...
            if ( glb != nullptr )
                MPI_Reduce( lc->data(), glb->data(), szMsg, Type_MPI<typename K::value_type>::mpi_type(), op, root,
                            com );
            else
        ...
        static void gather( const MPI_Comm &com, const K &loc, K *glob, int root )
        {
            ...
            if ( std::is_base_of<std::vector<typename K::value_type, typename K::allocator_type>, K>::value ) {
                ...
                if ( glb != nullptr ) {
                    if ( szMsg > glb->size() ) {
                        std::vector<typename K::value_type, typename K::allocator_type>( szMsg*sz ).swap( *glb );
                    }
                }
            } else {
                if ( rank == root )
                    glb = new std::vector<typename K::value_type, typename K::allocator_type>( szMsg*sz );
                else
                    glb = nullptr;
                lc = new std::vector<typename K::value_type, typename K::allocator_type>( szMsg );
                std::copy( loc.begin(), loc.end(), lc->begin() );
            ...
            if ( glb != nullptr )
                MPI_Gather( lc->data(), szMsg, Type_MPI<typename K::value_type>::mpi_type(), 
                           glb->data(), szMsg, Type_MPI<typename K::value_type>::mpi_type(), root, com );
            else
                MPI_Gather( lc->data(), szMsg, Type_MPI<typename K::value_type>::mpi_type(),
                               nullptr, szMsg, Type_MPI<typename K::value_type>::mpi_type(), root, com );
            if ( rank == root )
                if ( !std::is_base_of<std::vector<typename K::value_type, typename K::allocator_type>, K>::value ) {
                    std::copy( glb->begin(), glb->end(), glob->begin() );
                    delete glb;
