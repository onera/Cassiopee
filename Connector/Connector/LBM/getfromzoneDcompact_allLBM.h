	 ...
	 if(iscalcMacro_LBM==1){
	   ...
	   ipt_macros_localD[nd] = K_PYTREE::getValueAF(t, hook);
	 }

	 if(isIBM==1 || lbm_octree==1){
	   t                     = K_PYTREE::getNodeFromName1(sol, "Qneq_1" );
	   ipt_Qneq_localD[nd]   = K_PYTREE::getValueAF(t, hook);
	 }
	 if(lbm_octree==1){
	   t                     = K_PYTREE::getNodeFromName1(sol, "Q_1_M1" );
	   ipt_Qm1_localD[nd]    = K_PYTREE::getValueAF(t, hook);

	   t                        = K_PYTREE::getNodeFromName1(sol,"Density_M1" );
	   ipt_macrosm1_localD[nd]= K_PYTREE::getValueAF(t, hook);
	 }
	 
	 if(isneedQstar==1){
	   t                     = K_PYTREE::getNodeFromName1(sol, "Qstar_1" );
	   ipt_Qstar_localD[nd]  = K_PYTREE::getValueAF(t, hook);
	 }
       }
       else {
	 ipt_roD[nd]           = NULL;
	 ipt_macros_localD[nd] = NULL;
	 ipt_Qneq_localD[nd]   = NULL;
	 ipt_Qstar_localD[nd]  = NULL;
       } 
