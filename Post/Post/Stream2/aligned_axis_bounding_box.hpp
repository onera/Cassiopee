...
namespace K_POST
{
     ...
     * 
     * Par convention, on prendra pour repère le repère suivant ( i : gauche-droite, j : bas-haut, k : derrière-devant )
     * 
     *               j            
     *             ­  ⬆               
     *               O ⮕ i
     ...
     *           k
     *
     * de sorte que le sommet de la boîte ayant les plus petites coordonnées soit le plus à gauche, 
     * le plus derrière et le plus bas.
     */
    static constexpr const double tolerance_bbox = 1.E-14;
    struct aligned_axis_bounding_box
    {
        point3d p_min; /// Le point le plus à gauche, derrière et en bas; 
        point3d p_max; /// Le point le plus à droite, devant et en haut

        /// Constructeurs et destructeur
        //@{
        /// Constructeur par défaut
        aligned_axis_bounding_box() = default;
        /// Constructeur prenant les coordonnées d'un nuage de points pour calculer la boite englobante
        aligned_axis_bounding_box( const vector_view<const double>& xcoords, const vector_view<const double>& ycoords,
                                   const vector_view<const double>& zcoords )
        {
            ...
            p_min = {(*minmax_x.first)-tolerance_bbox, (*minmax_y.first)-tolerance_bbox, (*minmax_z.first)-tolerance_bbox};
            p_max = {(*minmax_x.second)+tolerance_bbox, (*minmax_y.second)+tolerance_bbox, (*minmax_z.second)+tolerance_bbox};
        }
         ...
         * 
         * Transforme la boîte englobante en chaîne de caractère facile à lire par un humain
         */
        explicit operator std::string() const
        {
            std::ostringstream sout;
            sout << "[ " << std::string(this->p_min) << ", " << std::string(this->p_max) << " ]";
            return sout.str();
        }
        //@}
         ...
         *
         * @return     Renvoie true si le point est dans la boîte englobante, faux sinon
         */
         ...
         *  
         *
         * @return Retourne la longueur, la hauteur et la profondeur de la boîte
         */
