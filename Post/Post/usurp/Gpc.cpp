...
void gpc_write_tristrip(FILE *fp, gpc_tristrip *p)
{
  int c, v;

  fprintf(fp, "%d\n", p->num_strips);
  for (c= 0; c < p->num_strips; c++)
  {
    fprintf(fp, "%d\n", p->strip[c].num_vertices);

    for (v= 0; v < p->strip[c].num_vertices; v++)
      fprintf(fp, "% .*lf % .*lf\n",
              DBL_DIG, p->strip[c].vertex[v].x,
              DBL_DIG, p->strip[c].vertex[v].y);
  }
}
                      ...
                      gpc_polygon *result)
{
  ...
  int            c, v, contributing=-1, search, scanbeam= 0, sbt_entries= 0;
  int            vclass, bl=0, br=0, tl=0, tr=0;
  double        *sbt= NULL, xb, px, yb, yt, dy=0., ix, iy;
  ...
  while (scanbeam < sbt_entries)
  {
        ...
        if (contributing)
        {
          ...
          case ELI:
            if (edge->outp[BELOW] == NULL){
               fprintf(stderr," ERROR! edge->outp[BELOW] == NULL.\n");
               fprintf(stderr," Exiting gpc_polygon_clip.\n");
         ...
         && (e1->bundle[ABOVE][CLIP] || e1->bundle[ABOVE][SUBJ]))
	{
	  ...
	  e0->bside[SUBJ]= !e0->bside[SUBJ];
        ...
        if (e0->next == NULL) {
               fprintf(stderr," ERROR! e0->next == NULL.\n");
               fprintf(stderr," Exiting gpc_polygon_clip.\n");
               result->num_contours = -1;
               result->hole= NULL;
               result->contour= NULL;
