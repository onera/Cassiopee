...
# local widgets list
WIDGETS = {}; VARS = []
listWalls = ['slip', 'noslip', 'Log', 'Musker', 'SA', 'TBLE', 'MuskerLinear','SALinear']

# Set IBM data in zone
# if zone is 1D STRUCT or BAR: remesh
def _setDataInZone(z, bLocal, snear, ibctype, ibctype2, dfar, inv):
...
def setData():
    ...
    for nz in nzs:
        ...
        bLocal = CTK.t[2][nob]
        z      = bLocal[2][noz]
        _setDataInZone(z, bLocal, snear, ibctype, ibctype2, dfar, inv)
...
def getData():
    ...
    if zone is not None:
        ...
        if n is not None:
            val = Internal.getValue(n)
            VARS[16].set(val)
            if val in listWalls: VARS[1].set("wall")
            else:                VARS[1].set(val)
            setMode()
        else:
            VARS[1].set('None')
...
def createApp(win):
    ...
    V = TK.StringVar(win); V.set('coarse(0)'); VARS.append(V)
    # -16- IBC type - for second scroll down menu (e.g. wall - slip, no slip, etc.)
    ...
    B = TTK.OptionMenu(wall, VARS[16], 'slip', 'noslip', 'Log', 'Musker', 'SA', 'TBLE', 'MuskerLinear','SALinear')
