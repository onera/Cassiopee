...
def createPyTreeForIBMWallFieldsExtraction(donorPoints_numpy, wallPoints_numpy, augState_numpy, discSelectionParaDict):
    ##Serial functions
    ...
    Internal.newDataArray("Pressure"          , value = augState_numpy[:,5+pdeList.index(discSelectionParaDict["PDE"])//2][idx_sorted], parent = FS)
    Internal.newDataArray("Temperature"       , value = augState_numpy[:,6+pdeList.index(discSelectionParaDict["PDE"])//2][idx_sorted], parent = FS)
    if discSelectionParaDict["PDE"] != 'Euler':
        Internal.newDataArray("ViscosityMolecular", value = augState_numpy[:,8+pdeList.index(discSelectionParaDict["PDE"])//2][idx_sorted], parent = FS)
    
    for i,varName in enumerate(listVariables): Internal.newDataArray('Velocity'  +varName      , value = augState_numpy[:,i+1][idx_sorted]/(augState_numpy[:,0][idx_sorted]), parent = FS)
    for i,varName in enumerate(listVariables): Internal.newDataArray('Coordinate'+varName+'_PW', value = numpy.ravel(wallPoints_numpy[:,i])[idx_sorted] , parent = FS)
    for i,varName in enumerate(listVariables): Internal.newDataArray('Coordinate'+varName+'_PI', value = numpy.ravel(donorPoints_numpy[:,i])[idx_sorted], parent = FS)
...
def computeBoundaryQuantities(zw, dictReferenceQuantities, dim=3, reorderFlag=False, invertYZ=False, verbose=False, time=-1):
    ...
    for lst in [res, res2, PressureCoef, FrictionCoef]:
...
## Their names suggest it is for the local IBMS but without a test case I do not recommend refactoring them (just yet).
