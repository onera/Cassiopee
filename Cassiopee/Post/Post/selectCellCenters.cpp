...
{
  PyObject* arrayNodes; PyObject* taga; PyObject* PE;
  PyObject* arrayCenters = NULL;
  E_Int cleanConnectivity;
  ...
  if (nargs == 4)
  {
    if (!PYPARSETUPLE_(args, OOO_ I_,
                       &arrayNodes, &taga, &PE, &cleanConnectivity))
      return NULL;
  }
  else
  {
    if (!PYPARSETUPLE_(args, OOOO_ I_,
		                   &arrayNodes, &arrayCenters, &taga, &PE, &cleanConnectivity))
      return NULL;
  }
...
#pragma omp parallel default(shared)
    {
      ...
      for (E_Int i = 0; i < nes[ithread]; i++)
      {
        ...
        for (E_Int k = 1; k <= nfldC; k++)
        {
          (*foutC)(i+p,k) = ftcenter[i*nfldC+(k-1)];
        }
      }
    }
    ...
    if (cleanConnectivity == 1 && posx > 0 && posy > 0 && posz > 0)
      tpln = K_CONNECT::V_cleanConnectivity(varString, *fout, *acn, eltType, 1.e-10);
    else tpln = K_ARRAY::buildArray3(*fout, varString, *acn, eltType, api);
    if (arrayCenters != NULL)
    {
  ...
  else // elements NGON
  {
    FldArrayF* fout = new FldArrayF(*f);
    FldArrayF* foutC = NULL;
    if (arrayCenters != NULL) foutC = new FldArrayF(*fC);  // pour recuperer la solution aux centres 
    ...
    if (PE != Py_None)
    {
      ...
      if (res == 0)
      {
        ...
        PyErr_SetString(PyExc_TypeError, "selectCellCenters: PE numpy is invalid.");
        return NULL;
      }
