...
//=============================================================================
PyObject* K_POST::enforceIndicatorForCoarsestLevel(PyObject* self, 
                                                   PyObject* args)
{
  ...
  if (res != 2) 
  {
    PyErr_SetString(PyExc_TypeError, 
                    "enforceIndicatorForCoarsestLevel: array must be unstructured.");
  ...
  //{
  //  PyErr_SetString(PyExc_TypeError, 
  //                  "enforceIndicatorForCoarsestLevel: array must be HEXA or QUAD.");
  ...
  {
    PyErr_SetString(PyExc_TypeError,
                    "enforceIndicatorForCoarsestLevel: coordinates not found in array.");
    RELEASESHAREDU(octree, f, cn); return NULL;
  }
  posx++; posy++; posz++;
  // Verif indicator
  ...
  {
    PyErr_SetString(PyExc_TypeError,
                    "enforceIndicatorForCoarsestLevel: indic array must be structured.");
    RELEASESHAREDU(octree, f, cn); return NULL;
  }
  ...
  if (posi == -1) 
  {
    ...
    printf("Warning: enforceIndicatorForCoarsestLevel: no refinement indicator given. Nothing done."); 
    return indicator;
  }
  posi++;
  ...
  {
    RELEASESHAREDB(resi, indicator, fi, cni); RELEASESHAREDU(octree, f, cn); 
    printf("Warning: enforceIndicatorForCoarsestLevel: refinement indicator size must be equal to the number of elements. Nothing done."); 
    return indicator;
  }
  /*-----------FIN DES VERIFS ------------------*/
  E_Int api = f->getApi();
  ...
  E_Float dhmax = -K_CONST::E_MAX_FLOAT;
  FldArrayF dht(nelts);
  ...
  for (E_Int et = 0; et < nelts; et++)
  {
    ...
    dhmax = K_FUNC::E_max(dhtp[et], dhmax);
  }
  E_Float eps = 1.e-10;
  for (E_Int et = 0; et < nelts; et++)
  {
    if (K_FUNC::fEqualZero(dhtp[et]-dhmax, eps) == true) 
      indict[et] = -3000.;
  }
