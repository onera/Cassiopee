...
CurvesToSurface::CurvesToSurface(std::vector<Handle(Geom_Curve) > const& splines_vector,
...
void CurvesToSurface::SetMaxDegree(int degree)
{
    assert( degree > 0 );
    _maxDegree = degree;
    Invalidate();
...
void CurvesToSurface::CalculateParameters(std::vector<Handle(Geom_BSplineCurve)> const& splines_vector)
{
