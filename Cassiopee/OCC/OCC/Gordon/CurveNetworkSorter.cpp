...
CurveNetworkSorter::CurveNetworkSorter(const std::vector<Handle (Geom_Curve)> &profiles,
...
void CurveNetworkSorter::swapProfiles(size_t idx1, size_t idx2)
{
    if (idx1 == idx2) {
        return;
    }
...
void CurveNetworkSorter::Perform()
{
    if (m_hasPerformed) {
        return;
    }
...
size_t CurveNetworkSorter::NProfiles() const
{
    return m_profiles.size();
}

size_t CurveNetworkSorter::NGuides() const
{
    return m_guides.size();
}

const std::vector<std::string> &CurveNetworkSorter::ProfileIndices() const
{
    return m_profIdx;
}

const std::vector<std::string> &CurveNetworkSorter::GuideIndices() const
{
    return m_guidIdx;
}
