...
PyObject* K_CONVERTER::convertStruct2NGon(PyObject* self, PyObject* args)
{
  ...
  E_Int ni1 = K_FUNC::E_max(1, E_Int(ni)-1);
  E_Int nj1 = K_FUNC::E_max(1, E_Int(nj)-1);
  E_Int nk1 = K_FUNC::E_max(1, E_Int(nk)-1);
  ...
  E_Int k1 = pow(2, dim0-1) + shift;
  E_Int k2 = 2*dim0 + shift;
  ...
  #pragma omp parallel
  {
    ...
    {
      #pragma omp for nowait
      for (E_Int i = 0; i < nfaces; i++) indPG2[i] = k1*i;
      #pragma omp for nowait
      for (E_Int i = 0; i < ncells; i++) indPH2[i] = k2*i;
    }
