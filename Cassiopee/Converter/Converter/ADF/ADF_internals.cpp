...
***********************************************************************/
static const char     *file_header_tags[] = {
...
void ADFI_find_file(char *parentfile, char *filename, int *error_return)
{
  ...
  if (cgio_find_file(parentfile, filename, 1, sizeof(pathname), pathname)) {
...
int				i ;
// Modified Cassiopee (long->cglong)
unsigned cglong			size ;
#endif
...
#else
  f_ret = file_open( file, f_mode | O_RDONLY | O_NOATIME, 0666);
  if (f_ret < 0) // if UID file != UID process O_NOATIME fails
    f_ret = file_open( file, f_mode | O_RDONLY, 0666);
#endif
}
