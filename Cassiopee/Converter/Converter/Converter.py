...
def convertArray2Tetra1__(array, arrayC=[], split='simple'):
    ...
    if split == 'simple': # no points added
        if t == 'STRUCT':
            ...
            group1 = ['NODE', 'BAR', 'TRI', 'TETRA']
            group2 = ['QUAD', 'PYRA', 'PENTA', 'HEXA']
            eltNames = t.split(',')
            center = all(eltName.endswith('*') for eltName in eltNames)
            eltNamesN = [eltName[:-1] for eltName in eltNames if eltName.endswith('*')]
