...
# --- Join two ME
# with fields at centers: tri-quad-tri
#                              |
#                             tri
...
# --- Join with a list of zones
# NGON with fields at centers
a = G.cartNGon((0.,0.,0.), (0.1,0.1,0.1), (5,10,4), api=3)
b = G.cartNGon((0.4,0.,0.), (0.1,0.1,0.1), (5,10,4), api=3)
c = G.cartNGon((0.8,-0.1,0.), (0.1,0.1,0.1), (5,10,4), api=3)
d = G.cartNGon((1.2,0.1,0.), (0.1,0.1,0.1), (5,10,4), api=3)
C._initVars(a, 'F', 1.); C._initVars(b, 'F', 2.)
C._initVars(c, 'F', 3.); C._initVars(d, 'F', 4.)
C._initVars(a, 'centers:G={centers:CoordinateX}'); C._initVars(b, 'centers:G={centers:CoordinateX}')
C._initVars(c, 'centers:G={centers:CoordinateX}'); C._initVars(d, 'centers:G={centers:CoordinateX}')
...
# 4 BEs with non-consecutive repeat
# with fields at centers: hexa - pyra
#                          |      |
#                        penta - hexa
a = G.cartHexa((0.,0.4,0.), (0.1,0.1,0.1), (5,5,5))
...
# 2 BEs and 1 ME with non-consecutive repeat
# with fields at centers: hexa - (pyra - hexa) - pyra
a = G.cartHexa((0.,0.,0.), (0.1,0.1,0.1), (5,5,5))
b = G.cartPyra((0.4,0.,0.), (0.1,0.1,0.1), (5,5,5))
c = G.cartHexa((0.8,0.,0.), (0.1,0.1,0.1), (5,5,5))
d = G.cartPyra((1.2,0.,0.), (0.1,0.1,0.1), (5,5,5))
C._initVars(a, 'F', 1.); C._initVars(b, 'F', 2.)
C._initVars(c, 'F', 3.); C._initVars(d, 'F', 4.)
C._initVars(a, 'centers:G={centers:CoordinateX}+{centers:CoordinateY}')
C._initVars(b, 'centers:G={centers:CoordinateX}+{centers:CoordinateY}')
C._initVars(c, 'centers:G={centers:CoordinateX}+{centers:CoordinateY}')
C._initVars(d, 'centers:G={centers:CoordinateX}+{centers:CoordinateY}')
b = T.join(b, c); a = T.join([a, b, d]); t = C.newPyTree(["Base", a])
