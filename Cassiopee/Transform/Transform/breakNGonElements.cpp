                                         ...
                                         char* varString)
{
  ...
  // Input NGon is either 2D or 3D and can be broken into list of BEs + NGon
  // Vertex masks for the resulting BEs and NGon connectivities, where 1 means
  // that this vertex belongs to this connectivity. NGon listed in position 0.
  std::vector<std::vector<E_Int> > vMask(nbuckets);
  for (E_Int ic = 0; ic < nbuckets; ic++) vMask[ic].resize(npts, 0);
  ...
  #pragma omp parallel
  {
    ...
    for (E_Int i = 0; i < nelts; i++)
    {
      ...
      if (dim == 2)
      {
        ...
        {
          // Tag vertices
          for (E_Int v = 0; v < nvpe; v++) vMask[1][vertices[v]-1] = 1;
      ...
      else if (nf == 4)  // TETRA
      {
        ...
        // Vertex order may have changed - update cEVNGon
        for (E_Int v = 0; v < nvpe; v++) vMask[3][vertices[v]-1] = 1;
        vertices[0] = vert1; vertices[1] = vert2; vertices[2] = vert3;
        vertices[3] = vert4;
      ...
      else if (nf == 5)  // PENTA, PYRA or NGon
      {
        ...
        if (nbnodes == 16)  // PYRA
        {
          ...
          // Vertex order may have changed - update cEVNGon
          for (E_Int v = 0; v < nvpe; v++) vMask[4][vertices[v]-1] = 1;
          vertices[0] = vert1; vertices[1] = vert2; vertices[2] = vert3;
        ...
        else if (nbnodes == 18) // PENTA
        {
          ...
          // Vertex order may have changed - update cEVNGon
          for (E_Int v = 0; v < nvpe; v++) vMask[5][vertices[v]-1] = 1;
          vertices[0] = vert1; vertices[1] = vert2; vertices[2] = vert3;
      ...
      else if (nf == 6) // HEXA
      {
        ...
        // Vertex order may have changed - update cEVNGon
        for (E_Int v = 0; v < nvpe; v++) vMask[6][vertices[v]-1] = 1;
        vertices[0] = vert1; vertices[1] = vert2; vertices[2] = vert3;
        vertices[3] = vert4; vertices[4] = vert5; vertices[5] = vert6;
      ...
      else  // element remains an NGon
      {
        ...
        {
          E_Int* face = cNG.getFace(elt[j]-1, nv, ngon, indPG);
          for (E_Int v = 0; v < nv; v++) vMask[0][face[v]-1] = 1;
        }
  ...
  // Build output list of BE
  if (nptsME > 0)
  {
    // Build new uncompressed eltType from connectivities that have at least
    // one element
    std::vector<const char*> tmp_eltType2;
    tmp_eltType2.push_back("TRI");
    tmp_eltType2.push_back("QUAD");
    tmp_eltType2.push_back("TETRA");
    tmp_eltType2.push_back("PYRA");
    tmp_eltType2.push_back("PENTA");
    tmp_eltType2.push_back("HEXA");
    ...
    for (E_Int ic = 1; ic < nbuckets; ic++)  // from TRI (1) to HEXA (6)
    {
      ...
      tpl = K_ARRAY::buildArray3(nfld, varString, npts2[ic],
                                 tmp_nepc2[ic], tmp_eltType2[ic-1], false, api);
      FldArrayF* f2; FldArrayI* cn2;
      K_ARRAY::getFromArray3(tpl, f2, cn2);
  ...
  vMask.resize(1); vMask.shrink_to_fit();
  ...
  if (npts2[0] > 0)
  {
    ...
    tpl = K_ARRAY::buildArray3(nfld, varString, npts2[0], nelts2,
    ...
    for (E_Int tid = 0; tid < nthreads; tid++)
    {
      ...
      for (size_t e = 0; e < loc_tindirNG.size(); e++)
      {
        ...
        for (E_Int j = 0; j < nf; j++)
        {
          ...
          {
            ind = face[v] - 1;
            ngon2[ind3+v+shift] = vMask[0][ind];
          }
          ind3 += nv + shift;
