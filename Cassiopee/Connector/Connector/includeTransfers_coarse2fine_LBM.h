//======================================================
// Transfert coarse grid to fine grid LBM
//======================================================

E_Float taug_c = ipt_param_realR[NoD][LBM_TAUG];
E_Float taug_f = ipt_param_realR[NoR][LBM_TAUG];
...
E_Float scale_c2f = 0.5*taug_f/taug_c;
...
if (nb_dist==19)
{
  ...
  for (E_Int noind = pt_deb; noind < pt_fin; noind++)
  {
    ...
    vectOfRcvFields[shift_q][indR] = eq1 + offeq1 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 1][indR] = eq2 + offeq2 * scale_c2f + 0.5 * flag_psi * psi;

    // Q 3
    eq3 = eq2 - 6 * Ro * w2 * (vit - 3. * coef2);
    offeq3 = vectOfRcvFields[shift_q + 2][indR] - eq3 - flag_psi * psi;
    vectOfRcvFields[shift_q + 2][indR] = eq3 + offeq3 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 3][indR] = eq4 + offeq4 * scale_c2f + 0.5*flag_psi*psi;

    // Q 5
    eq5 = eq4 - 6 * Ro * w2 * (vit - 3 * coef1);
    offeq5 = vectOfRcvFields[shift_q + 4][indR] - eq5 - flag_psi * psi;
    vectOfRcvFields[shift_q + 4][indR] = eq5 + offeq5 * scale_c2f + 0.5*flag_psi*psi;
    ...
    vectOfRcvFields[shift_q + 5][indR] = eq6 + offeq6 * scale_c2f + 0.5*flag_psi*psi;

    // Q 7
    eq7 = eq6 - 6 * Ro * w2 * (vit - 3 * coef3);
    offeq7 = vectOfRcvFields[shift_q + 6][indR] - eq7 - flag_psi * psi;
    vectOfRcvFields[shift_q + 6][indR] = eq7 + offeq7 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 7][indR] = eq8 + offeq8 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 8][indR] = eq9 + offeq9 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 10
    eq10 = eq9 - Ro * w3 * (6 * vit2 - 9 * herm_alt);
    offeq10 = vectOfRcvFields[shift_q + 9][indR] - eq10 - flag_psi * psi_alt;
    vectOfRcvFields[shift_q + 9][indR] = eq10 + offeq10 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 11
    eq11 = eq8 - Ro * w3 * (6 * vit - 9 * herm);
    offeq11 = vectOfRcvFields[shift_q + 10][indR] - eq11 - flag_psi * psi;
    vectOfRcvFields[shift_q + 10][indR] = eq11 + offeq11 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 11][indR] = eq12 + offeq12 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 12][indR] = eq13 + offeq13 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 14
    eq14 = eq13 - Ro * w3 * (6 * vit2 - 9 * herm_alt);
    offeq14 = vectOfRcvFields[shift_q + 13][indR] - eq14 - flag_psi * psi_alt;
    vectOfRcvFields[shift_q + 13][indR] = eq14 + offeq14 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 15
    eq15 = eq12 - Ro * w3 * (6 * vit - 9 * herm);
    offeq15 = vectOfRcvFields[shift_q + 14][indR] - eq15 - flag_psi * psi;
    vectOfRcvFields[shift_q + 14][indR] = eq15 + offeq15 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 15][indR] = eq16 + offeq16 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 16][indR] = eq17 + offeq17 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 18
    eq18 = eq17 - Ro * w3 * (6 * vit2 - 9 * herm_alt);
    offeq18 = vectOfRcvFields[shift_q + 17][indR] - eq18 - flag_psi * psi_alt;
    vectOfRcvFields[shift_q + 17][indR] = eq18 + offeq18 * scale_c2f + 0.5 * flag_psi * psi_alt;

    // Q 19
    eq19 = eq16 - Ro * w3 * (6 * vit - 9 * herm);
    offeq19 = vectOfRcvFields[shift_q + 18][indR] - eq19 - flag_psi * psi;
    vectOfRcvFields[shift_q + 18][indR] = eq19 + offeq19 * scale_c2f + 0.5 * flag_psi * psi;
...
else if (nb_dist==27)
{
  ...
  for (E_Int noind = pt_deb; noind < pt_fin; noind++)
  {
    ...
    vectOfRcvFields[shift_q + 8][indR] = eq9 + offeq9 * scale_c2f + 0.5 * flag_psi * psi;

    // Q 10
    eq10 = eq9 - 6. * Ro * w3 * (vit2 - 1.5 * herm_alt +0.75 * herm3_alt);
    offeq10 = vectOfRcvFields[shift_q + 9][indR] - eq10 - flag_psi * psi;
    vectOfRcvFields[shift_q + 9][indR] = eq10 + offeq10 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 12][indR] = eq13 + offeq13 * scale_c2f + 0.5 * flag_psi * psi;

    // Q 14
    eq14 = eq13 - 6 * Ro * w3 * (vit2 - 1.5 * herm_alt +0.75 * herm3_alt);
    offeq14 = vectOfRcvFields[shift_q + 13][indR] - eq14 - flag_psi * psi;
    vectOfRcvFields[shift_q + 13][indR] = eq14 + offeq14 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 16][indR] = eq17 + offeq17 * scale_c2f + 0.5 * flag_psi * psi;

    // Q 18
    eq18 = eq17 - 6 * Ro * w3 * (vit2 - 1.5 * herm_alt +0.75 * herm3_alt);
    offeq18 = vectOfRcvFields[shift_q + 17][indR] - eq18 - flag_psi * psi;
    vectOfRcvFields[shift_q + 17][indR] = eq18 + offeq18 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 19][indR] = eq20 + offeq20 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 20][indR] = eq21 + offeq21 * scale_c2f + 0.5 * flag_psi * psi;
    ...
    vectOfRcvFields[shift_q + 21][indR] = eq22 + offeq22 * scale_c2f + 0.5 * flag_psi * psi;
