...
PyObject* K_OCC::fixShape(PyObject* self, PyObject* args)
{
  ...
  */
  ...
  // Face rebuild example
  /*
  ShapeBuild_ReShape builder;
  build.Remove(face);
  *newshp = builder.Apply();
  */

  // unify edges
  ShapeUpgrade_UnifySameDomain USD(*shp, Standard_True, Standard_True, Standard_False); // UnifyFaces mode on, UnifyEdges mode on, ConcatBSplines mode on.
  USD.Build();
  *newshp = USD.Shape();
