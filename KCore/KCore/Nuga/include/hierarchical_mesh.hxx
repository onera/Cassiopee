...
class hierarchical_mesh
{
  public:
    ...
    ///
    inline void extract_loop_plan(E_Int PGi, bool reverse, pg_arr_t& plan) const; //todo VD : � impl�menter
    ///
...
void hierarchical_mesh<K_MESH::Hexahedron, DIR, ngon_type>::__init()
{
  ...
  for (E_Int i=0; i < nb_phs; ++i)
  {
    ...
    // alexis : utiliser pg et qopp pour r�ordonner
  }
  
  // promote eventually to layer type + 2nd reorder
  for (E_Int i=0; i < _F2E.cols(); ++i)
  {
    // alexis : si i n'est pas une fronti�re => continue
