...
#ifndef NUGA_ADAPTOR_OMP_HXX
#define NUGA_ADAPTOR_OMP_HXX
...
class adaptor_omp : public omp_algo<hmesh_t, E_Int>
{
public:

  using parent_t = omp_algo < hmesh_t, E_Int>;
  ...
  ) override
  {
    ...
    for (auto& it : rid_to_list)
    {
      ...
      {
        /*typename hmesh_t::pg_arr_t*/K_FLD::IntArray p; //only implemented for IntArray
  ...
  bool run_with_data (const std::vector<hmesh_t*>& hmeshes, const std::map<int, std::map<int, K_FLD::IntArray>> & zid_to_data) override
  {
...
#pragma omp parallel for reduction ( || : has_omp_changes) if(PARA == COARSE_OMP)         
    for (E_Int i = 0; i < NBZ; ++i)
    {
      ...
      auto it = zid_to_data.find(hmeshes[i]->zid);
      if (it == zid_to_data.end()) continue;
