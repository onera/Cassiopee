 ...
 K_CONT_DEF::int_vector_type& ancestors)
{
  ...
  if (mode.ignore_coincident_nodes) K_CONNECT::IdTool::init_inc(nids, pos.cols());
...
//        }
    ...
    //}
      ...
      err = __iterative_run (mesher, &pi, &ci2, hnodes, data);
      ...
      {
        for (size_t n = 0; n < data.hnids.size(); ++n)
        {
          if ((size_t)data.hnids[n] != n) nids[revIDs[n]] = revIDs[data.hnids[n]];
        }
      }
...
//#endif
  ...
  if (mode.ignore_coincident_nodes)
    K_FLD::IntArray::changeIndices(connectOut, nids);
 ...
 K_FLD::FloatArray& crd, K_FLD::IntArray& cnt, std::vector<E_Int>& oids)
{
  ...
  K_CONNECT::MeshTool::compact_to_mesh(pos, cnt0, crd, cnt1, oids);
 ...
 K_FLD::IntArray& connect, K_CONT_DEF::int_vector_type& ancestors, K_CONT_DEF::bool_vector_type& xc)
{
  ...
  // Get the common nodes.
  {
    ...
    K_CONNECT::MeshTool::getNonManifoldEdges(connect, connectXC); // Common edges.
...
(K_FLD::IntArray& connect, std::vector<E_Int>& ancestors, K_CONT_DEF::bool_vector_type& xc)
{
  ...
  bool has_duplis = K_CONNECT::MeshTool::detectDuplicated(connect, dupIds, false /*strict orient*/);
  ...
  algo_type::BoundToEltType::const_iterator itE;
  K_FLD::ArrayAccessor<K_FLD::IntArray> actv(connect);
  K_CONNECT::EltAlgo<K_MESH::Triangle>::getBoundToElements(actv, E_to_T);
 ...
 const std::vector<T3> & elts, bool localid, std::vector<E_Int>* colors)
{
  ...
  if (localid)
    K_CONNECT::MeshTool::compact_to_mesh(coord1, cOut, newIDs);
...
(const K_FLD::FloatArray& pos, K_FLD::IntArray::const_iterator pS, E_Int Ni, E_Int Nj)
{
  ...
  K_CONNECT::MeshTool::compact_to_mesh(p, toto, nids);
...
TRI_Conformizer<DIM>::drawT3(const K_FLD::FloatArray& pos, const K_FLD::IntArray& connect, E_Int ith_elt, bool compact)
{
  ...
  if (compact)
  {
    ...
    K_CONNECT::MeshTool::compact_to_mesh(p, cOut, newIds);
