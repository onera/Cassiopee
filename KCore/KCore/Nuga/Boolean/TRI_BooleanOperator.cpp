...
TRI_BooleanOperator::get_2_minus_1(K_FLD::FloatArray& coord, K_FLD::IntArray& connect, std::vector<E_Int>& colors)
{
  ...
  K_CONNECT::MeshTool::flipT3(temp);
...
TRI_BooleanOperator::__compute_x_contour(std::set<K_MESH::NO_Edge>& hXC)
{
  ...
  K_CONNECT::MeshTool::getBoundary(connectC, connectCb);
...
(const K_FLD::FloatArray& coord, K_FLD::IntArray* connects, E_Int& S0, E_Int& j)
{
  ...
  // e.g. minN[1] is the index of the node having the lowest y-coordinate.
  {
    ...
    K_CONNECT::MeshTool::boundingBox(coord, connects[0], minN, maxN);
    K_CONNECT::MeshTool::boundingBox(coord, connects[1], minN1, maxN1);
 /*  
    for (E_Int k = 0; k < 3; ++k)
    {
...
(const K_FLD::IntArray& connect, E_Int S0, const std::set<K_MESH::NO_Edge>& hXC, std::vector<E_Int>& colors)
{
  ...
  K_CONNECT::EltAlgo<K_MESH::Triangle>::NeighbourType    neighbors;
  K_CONNECT::EltAlgo<K_MESH::Triangle>::getNeighbours(connect, neighbors);
  if (neighbors.empty())
    return 1;

  // Get the zones.
  K_CONNECT::EltAlgo<K_MESH::Triangle>::coloring(connect, neighbors, hXC, zones);
 ...
 std::vector<E_Int>& zones, std::map<E_Int, std::set<E_Int> >& zone_to_zones)
{
  ...
  K_CONNECT::EltAlgo<K_MESH::Triangle>::BoundToEltType bound_to_elts;
  K_FLD::ArrayAccessor<K_FLD::IntArray> actv(connect);
  K_CONNECT::EltAlgo<K_MESH::Triangle>::getBoundToElements(actv, bound_to_elts);
  K_CONNECT::EltAlgo<K_MESH::Triangle>::BoundToEltType::iterator it, itEnd(bound_to_elts.end());
...
(const K_FLD::IntArray& connect, const std::set<K_MESH::NO_Edge>& hXC, std::set<K_MESH::Edge>& hXCO)
{
  ...
  K_CONNECT::MeshTool::getBoundary(connectU, boundary);
...
TRI_BooleanOperator::__getCommonTriangles(K_FLD::IntArray& connectC)
{
  ...
  K_CONNECT::MeshTool::detectDuplicated(connectG, dupIds, false /*strict orient*/);
...
(const K_FLD::IntArray& connectInter, K_FLD::IntArray& connectIS, K_FLD::IntArray& connectIO)
{
  ...
  K_CONNECT::MeshTool::detectDuplicated(connectInter, nids, true/*strict orient*/);
  ...
  K_CONNECT::MeshTool::removeDuplicated(connectIO, nids, false/*strict orient*/);
...
{
  K_CONNECT::EltAlgo<K_MESH::Triangle>::BoundToEltType bound_to_elts;
  K_CONNECT::EltAlgo<K_MESH::Triangle>::BoundToEltType::const_iterator it;
