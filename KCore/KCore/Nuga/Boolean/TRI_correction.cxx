 ...
 E_Float tolerance)
{
  ...
  //
  K_CONNECT::MeshTool::detectDuplicated(connect, dupIds, false /*strict orient*/);
  ...
  K_CONNECT::MeshTool::computeNodeNormals(pos, connect, normals);
  ...
  for (size_t g = 0; g < contour_to_zones.size(); ++g)
  {
    ...
    for (std::set<E_Int>::const_iterator it = overlapz.begin(); it != overlapz.end(); ++it)
    {
      ...
      if (!same_orient)
        K_CONNECT::MeshTool::flipT3(connectZs[Zi]);
    }
  }
 ...
 K_CONT_DEF::int_vector_type& ancestors, K_CONT_DEF::bool_vector_type& xc, const K_FLD::FloatArray& pos, K_CONT_DEF::int_vector_type* priority)
{
  ...
  for (std::set<E_Int>::const_iterator it = oids.begin(); it != oids.end(); ++it)
  {
    ...
    for (it1 = col_to_connectC.begin(); it1 != col_to_connectC.end(); ++it1)
    {
      ...
      K_CONNECT::MeshTool::getBoundary(it1->second, connectB);
 ...
 std::vector<std::vector<E_Int> >& grouped_zones)
{
  ...
  for (size_t z = 0; z < connectZs.size(); ++z)
  {
    ...
    K_CONNECT::MeshTool::removeDuplicated(tmp, dids, false);

    K_CONNECT::MeshTool::getBoundary(tmp, connectb);
 ...
 const K_FLD::IntArray& connectZ2, const K_SEARCH::BbTree3D& treeZ2)
{
  ...
  K_CONNECT::MeshTool::getBoundary(connectZ1, connectb);
 ...
 const std::set<K_MESH::NO_Edge>& common_edges, std::vector< std::vector<E_Int> >& zones)
{
  ...
  K_CONNECT::EltAlgo<K_MESH::Triangle>::coloring(connect, neighbors, common_edges, colors);
...
)
{
  ...
  if (!swapKb.empty())
  {
    ...
    K_CONNECT::EltAlgo<K_MESH::Triangle>::getManifoldNeighbours (connectW, neighbors, false);//false : accept non manifold
