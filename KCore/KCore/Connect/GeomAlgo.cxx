...
void
K_CONNECT::GeomAlgo<ElementType>::neighboring
...
(const Vector_t<K_FLD::FloatArray*>& crds, const Vector_t<K_FLD::IntArray*>& cnts, bool outward)
{
  if (crds.empty() || cnts.empty())
   ...
   for (E_Int i=1; i < nb_zones; ++i)
   {
     ...
     K_CONNECT::MeshTool::removeDuplicated(*cnts[i], dupIds, false);
   }
   ...
   K_SEARCH::BBox3D* pool = new K_SEARCH::BBox3D[nb_elts];
   Vector_t<K_SEARCH::BBox3D*> boxes(nb_elts);
...
#pragma omp parallel for private(neighbors, orient, colors)
   for (E_Int i=0; i< nb_zones; ++i)
   {
     ...
     neighbors.clear();
     K_CONNECT::EltAlgo<K_MESH::Triangle>::getManifoldNeighbours(*cnts[i], neighbors, false);
     //b. call coloring
