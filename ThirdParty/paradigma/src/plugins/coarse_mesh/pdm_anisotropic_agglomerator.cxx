...
{
  /*
   * Remarque : fonction couteuse Il y a un tri d'un dictionnaire!
   */
  int numberOfFineCells = sizes[0];

  /* numberOfFineAnisotropicCompliantCells at the  beginning */
  int numberOfFineAnisotropicCompliantCells = sizes[7];
  int numberOfAnisotropicLinesPOne_size     = sizes[8];
  int agglomerationLines_size               = sizes[9];

  /* Parameter for anistoropic line */
   ...
   *     -> keys are the ratio Max to average (ratioArray[iCell]) and value
   *        the (global) index of the cell.
   */
  unordered_map<int, double> dictAnisotropicCell;
  ...
  /* Post treated the dictionary of anisotropic cells */
  if (!dictAnisotropicCell.empty()){
    ...
    // Sorting!
    multimap<double, int> sorted_anisotropicCells = flip_map(dictAnisotropicCell);
...
void agglomerate_Anisotropic_One_Level_without_list_lines(int  *sizes,
                                                          int  *fineAgglomerationLines_array_Idx,
                                                          int  *fineAgglomerationLines_array,
                                                          ...
                                                          int  *AnisotropicCompliantCoarseCells_array)
{
  ...
  /* Updata sizes for next agglomeration */
