                         ...
                         int     verbose_int)
{
  ...
  // number of fine (already) agglomerated cells
  numberOfFineAgglomeratedCells = 0;
  ...
  if(isAnisotropic){
    if(isFirstAgglomeration){
      ...
      isAnisotropicLines = computeAnisotropicLine(sizes,
                                                  ...
                                                  agglomerationLines,
                                                  verbose);
      // number of agglomeration lines +1
                                     ...
                                     int     verbose_int)
{
  ...
  if(isAnisotropic) {
    ...
    if (isAnisotropicLines){
      ...
      // Verbose
      // cout<<"tagglomerateOneLevel_v_Paradigma : "<<endl;
                                         ...
                                         bool                verbose)
{
  ...
  if(!dict_DistributionOfCardinalOfCoarseElements.empty()) {
    if( dict_DistributionOfCardinalOfCoarseElements.size()          != 1 ||
        dict_DistributionOfCardinalOfCoarseElements.count(goalCard) == 0){

      agglomerate_Isotropic_Second_Step_Correction(sizes,
                                                   matrixAdj_CRS_row_ptr,
                                                   matrixAdj_CRS_col_ind,
...
/**
 *
 * \brief This function finds the fine cells in the list listOfFineCells that defined the biggest distance of the covering tree.
 *
 * \param [in]   listOfFineCells
 * \param [in]   matrixAdj_CRS_row_ptr
 * \param [out]  matrixAdj_CRS_col_ind
 * \param [out]  agglomerationLinesIdx
 * \param [out]  isOnFineBnd
 *
 */
static unordered_map<int, queue<int> *> findSeedViaFrontalMethod(int         *sizes,
                                                          ...
                                                          int         *matrixAdj_CRS_col_ind)
{
  ...
  vector<int> colour = vector<int>(nbOfCells);
...
unordered_map<int, int> computation_Of_Neighbourhood(int                 seed,
                                                     int                 numberOfOrderOfNeighbourhood,
                                                     int                *matrixAdj_CRS_row_ptr,
                                                     int                *matrixAdj_CRS_col_ind,
                                                     int                 maxCard,
...
list<unordered_set<int>> partsList(vector<int> seq, int length) {
    ...
    int i = 1;
    int iMax = (int) pow(2, (int) seq.size()) - 1;
...
void agglomerate_Isotropic_Correction_SplitTooBigCoarseCellInTwo(
                                                                 int *sizes,
                                                                 vector<queue<int>> &listOfSeeds,
                                                                 unordered_map<int, unordered_set<int>> &dict_Coarse_Elem,
                                                                 unordered_map<int, unordered_set<int>> &dict_Card_Coarse_Cells,
    ...
    for (int iSize = maxCard + 1; iSize < maxSize + 1; iSize++) {
        if (dict_Card_Coarse_Cells.count(iSize) == 1) {
            ...
            for (int iCoarseCell: tmp_set_Coarse_Cell_Of_Size_iSize) {
                ...
                unordered_map<int, queue<int> *> dict_ConnectivityTree = findSeedViaFrontalMethod(localSizes, listOfFineCells, matrixAdj_CRS_row_ptr, matrixAdj_CRS_col_ind);
...
void agglomerate_Isotropic_Second_Step_Correction(int *sizes,
                                                 int* matrixAdj_CRS_row_ptr,
                                                 int* matrixAdj_CRS_col_ind,
                                                 double* matrixAdj_CRS_values,
                                                 double* volumes,
...
//                unordered_set<int> s = dict_CoarseCells[index];
        ...
        agglomerate_Isotropic_Correction_SplitTooBigCoarseCellInTwo(sizes, listOfSeeds,
                                                                    dict_CoarseCells,
                                                                    dict_CardCoarseCells,
                                                                    matrixAdj_CRS_row_ptr,
                                                                    matrixAdj_CRS_col_ind,
