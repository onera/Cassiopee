...
! integer                            :: flipped=0
#ifdef USE_TECPLOT
  integer,parameter                  :: visdouble=0,debugtecplot=0
  integer,parameter                  :: zero=0
  integer                            :: TecDat100,TecEnd100
  integer                            :: TecFil100,TecIni100
  integer                            :: TecNod100,TecZne100
#endif
...
!   open Tecplot file and write header information
    ...
    if (tecbinary) then
      ...
      if (have_qv) then
        allocate(varloc(5)); nalloc = nalloc + 1
        varloc = (/ 1,1,1,1,0 /)
      else
        allocate(varloc(4)); nalloc = nalloc + 1
        varloc = (/ 1,1,1,0 /)
      end if

      ierr = TecIni100 ( "usurp"//char(0), &
                         trim(tecvariables)//char(0), &
                         trim(pathname)//"usurp-triangles.plt"//char(0), &
...
!   copy the needed vertex coordinates
    do idir = 1,3
      ...
      if (tecbinary) then
#ifdef USE_TECPLOT
        ierr = TecDat100(num_used,v2,visdouble)
#endif
      else 
        write(44,*)v2
      end if
