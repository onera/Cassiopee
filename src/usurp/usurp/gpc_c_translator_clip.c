...
void gpc_c_translator_clip_(int *a, double *b)
{
   ...
   clip.contour = malloc(clip.num_contours * sizeof(gpc_vertex_list));
   clip.hole    = malloc(clip.num_contours * sizeof(int));
   ...
   for (ic=0; ic < clip.num_contours; ic++) {
       ...
       clip.contour[ic].vertex = malloc(clip.contour[ic].num_vertices * sizeof(gpc_vertex));
